...
- Result Log: COMPLETE. Mode: AFK — auto-dispatched. `src/aa_ma/enforce.py` (`FieldRead`, `read_enforced_field`, `read_milestone_status`, `read_step_status`, five canonical frozensets) with `tests/test_enforce.py`: rows 1-14 assert their contract verdict (rows 15-18 are block-structure rows — 16/17 landed in `test_grammar.py` at 5.0, 15/18 belong to `test_gate.py`). RED (ImportError) -> GREEN 24/24. **The candidate regex is deliberately wider than the accept regex** — `* Status:` and NBSP forms are *found and then refused* with the line quoted via `repr` (an NBSP shows as `\xa0`, which is the point of quoting); a narrower regex would have reported them absent, the fail-open shape rows 9-10 exist to close. One normalisation for every field: leading token, `*` stripped, upper-cased, then case-insensitive canonical membership returning the canonical spelling — so `Critical-Path: Data-Xform` reads `data-xform`. **Contract amended with a measurement**: the step set as pinned would refuse `SKIPPED`, which §5.2 of the command itself writes, and `DEFERRED` (3 corpus lines); both added, reference.md updated in place with the reason. `tui/parser.py` untouched; drift guard asserts the enforce sets against the TUI enums. Full pytest **891 passed**, ruff clean.
...
- Result Log: COMPLETE. Mode: AFK — auto-dispatched. `tests/test_gate.py`, 40 cases (corrected from "33" at the M5 §6.8 review — `pytest --collect-only` at e4d1433), RED at collection (`aa_ma.gate` does not exist). Corpus: the four `gate-scans` fixtures (one/two/no-active, styles with all 9 heading styles by number), the live plan, and inline cases for rows 6-17 plus `2a`/`3.5`, duplicate numbers, backslash titles, CRLF, the file-level rule, step-scope, SKIPPED/DEFERRED, and the CLI (JSON against a declared schema, `--help` naming all five codes, console-script declared). **Two decisions recorded before the code exists, each measured on the 28-file corpus** (0 orphan steps, 0 bare `##`, 0 CRLF — so neither costs a real plan anything): (a) row 16 'bare `##` closes the block' is completed by 'a sub-step outside every milestone block is unreadable (exit 2)' — otherwise closing the block silently hides PENDING steps, the false PASS the 3rd §6.8 pass flagged in 4.12; (b) step-level Status is validated only within the milestone being answered, since a gap in a *completed* milestone's step cannot change which milestone is ACTIVE, while milestone-level fields are validated file-wide per the contract's file-level rule. `jsonschema` (already installed transitively) declared in the dev group for the schema assertion.
...
- Result Log: COMPLETE. Mode: AFK — auto-dispatched. `src/aa_ma/gate.py` (`answer()`, `MilestoneRead`, `GateAnswer`, `GATE_JSON_SCHEMA`, `main()`) answers all seven questions over `grammar` + `enforce` + `plan_parsers`; `grammar.py` gains `has_unterminated_fence` (row 15). 5.2's 40 cases GREEN on the first run, including the live plan (`Milestone 5`, HARD, hook-modification, infra). `tests/test_gate_parity.py` is the behavioural no-second-parser guard: all 14 field rows driven through `enforce.read_enforced_field` and through the gate, readings asserted identical (valid -> same value; refused -> the primitive's own error text appears in the gate's errors); block choice asserted equal to `grammar.split_milestones` on all four fixtures; Audit-Profile agrees with `plan_parsers` on the 5 canonical values and the **one deliberate divergence** (`**infra**`: linter rejects, gate reads) is asserted so it stays known. **Mutation-verified, and it found a gap**: counting COMPLETE instead of PENDING fails 8; removing the fence check fails 1; removing the orphan check fails 1; but reading milestone fields from the *whole* block instead of the own-text before the first `###` survived every case, because own fields precede step fields in every fixture. Added the killing case (status-less milestone whose step has a Status -> exit 2 'no Status'); mutant now fails 1. Full pytest **953 passed**, ruff clean.
...
- Result Log: COMPLETE. Mode: AFK — auto-dispatched. `aa-ma-gate` console script (`[project.scripts]`, `aa_ma.gate:main`); `--format json|kv` — kv is one `key=value` per line with the value verbatim after the first `=`, because the bash callers have no JSON parser (`jq` is not a CI assumption here; `yq` is already absent) and a heading must reach §7.1's `grep -F` byte-exact; asserted with a title containing `=`, `\t` and `C:\dev\path`. All five codes reachable from fixtures and JSON validated against `GATE_JSON_SCHEMA` (`jsonschema`, declared in the dev group); `--help` names every code. **Interpreter-unavailable path is tested in bash, where it lives**: `aa_ma_gate` in `aa-ma-parse.sh` is a *launcher* (`uv run --project <forge-root> aa-ma-gate --format kv`, root resolved via `readlink -f` on the symlinked lib so it works from any consumer repo) that returns 127 with a BLOCKED line when uv is absent, **and** when uv is present but the tool does not start — `uv run` exits 2 on spawn failure, the same 2 the contract uses for 'unreadable', so the launcher checks its own output for `exit_code=` to tell them apart. Both cases bats-tested with PATH shims plus a non-vacuity case. A python3 fallback was considered and skipped (YAGNI: `uv` is this repo's stated toolchain and `verify-impl` already assumed it) — add one when a consumer without uv appears.
...
- Result Log: COMPLETE. Mode: AFK — auto-dispatched. §6.7 preamble replaced: one `aa_ma_gate` call answers every question, `case` on rc 1/2/3/* each `exit 1` with the gate's own `error=` lines echoed; `MILESTONE_TITLE`, `PENDING_IN_MILESTONE`, `CRITICAL_PATH_TASKS`, `PROTOTYPE_TASKS` and `GATE` all read with `aa_ma_gate_field`; condition 1's `# HALT` is `exit 1`. §7.1 reuses `${GATE}` from the preamble (no re-derivation). `verify-impl` Step 1 calls `aa_ma_gate --milestone N`, gains the missing `*)` arm, and reads `audit_profile` from kv instead of a `python -c` heredoc. **Also fixed while there**: `plan-verification` Angle 6 check #2 piped an undefined `$MILESTONE_BLOCK` into `parse_critical_path`, which read an empty block as OK — the check passed vacuously; now a gate call. `tests/hooks/aa-ma-gate-python.bats` **executes the shipped §6.7 text** (extracted by awk from the command file, run with `TASK_NAME` set against `.claude/dev/active/<name>/` dirs built from the fixtures): one-active -> PASS; two-active -> BLOCKED naming both; no-active -> BLOCKED; PENDING -> BLOCKED with count; Critical-Path without a *milestone-scoped* review entry -> BLOCKED, an entry naming a different milestone still BLOCKED, the right one -> PASS; `Gate: TYPO` -> BLOCKED quoting it; backslash title round-trips; dirty task dir -> non-zero, no PASS line; uv missing -> BLOCKED; and the live plan through the shipped text (currently BLOCKED: `6 sub-step(s) still PENDING in Milestone 5` — correct, and the first time the shipped gate has ever counted this plan's pending steps right). Dogfooded directly as well, same result.
...
- Result Log: COMPLETE. Mode: AFK — auto-dispatched. `aa-ma-gate --milestone N --step N.M` resolves the sub-step's Mode (own -> milestone -> `HITL`, with `step_mode_source` saying which) and the file is refused (exit 2, line quoted) before any Mode is emitted when it carries `Mode: TYPO`. §5.2 step 1.5 of the command file now carries an executable fence calling it, with `exit 1` on any non-zero rc — a sub-step whose Mode the gate cannot read is never dispatched. RED 5 -> GREEN in pytest; bats executes the shipped Mode fence for TYPO (BLOCKED, no `MODE=AFK`) and for own/inherited/default resolution. Dogfooded on this plan: 5.6 -> `AFK`/`step`, 5.9 -> `HITL`/`step`.
