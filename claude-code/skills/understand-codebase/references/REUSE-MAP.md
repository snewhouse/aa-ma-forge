...
### `codemem build` — structural index
- **When:** no `.codemem/index.db`, tier ≥ Standard, repo not pathologically huge. Not needed when
  you query through codemem's MCP tools — they build the index on first query.
- **How:** from the target repo's root, `uv run --project "$AA_MA_ROOT" codemem build` (`AA_MA_ROOT`
...
- **How:** `Agent(subagent_type="gsd-codebase-mapper", prompt=…)` four times, one per focus
  ...
  - In the prompt, include a `<required_reading>` block pointing at the codemem index (or `PROJECT_INDEX.json`, its fallback) if it exists.
...
### `codemem draw --write` — the living architecture doc (Deep tier)
...
- **When:** S1 (Standard) and the mapper agents (Deep) — use its `sg`
  ...
  maps, class hierarchies) for consistency. Prefer codemem's MCP tools (`who_calls`, `blast_radius`,
  `dependency_chain`, `search_symbols`, `file_summary`, `dead_code`, `diagram`); with codemem absent and
  `PROJECT_INDEX.json` present, `code-intelligence-index` is the equivalent fallback (codemem not configured).
...
- Deep tier, the living architecture doc: `<repo>/docs/architecture/**`, `<repo>/.codemem/` (the index), and one `.codemem/` line appended to `<repo>/.gitignore` if absent.
- via reused tools: `<repo>/.planning/codebase/**` (`gsd-map-codebase`, Deep).
