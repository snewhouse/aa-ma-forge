...
## [2026-09-21] Milestone Completion: Milestone 5 — Charting — `/aa-ma-chart` (Adaptation of wayfinder) + `--from-map`; release v0.14.0
- Status: COMPLETE
- Key outcome: `/aa-ma-chart` (chart/work modes) + `aa-ma-chart-guard.sh` (fog | claim | reclaim | from-map | import, bats 28/28) + `map-template.md` + `/aa-ma-plan --from-map <effort> [--dry-run]`; proven on the real effort `writing-for-agents-eval` (4 tickets resolved with Ste, 1 refused claim, dry-run handoff exact); file taxonomy 5+4, commands 13; ADR-0013 Implemented; v0.14.0 cut (b527241, tag ea6d7d1).
- Artifacts: `claude-code/commands/aa-ma-chart.md`, `claude-code/hooks/lib/aa-ma-chart-guard.sh`, `docs/templates/map-template.md`, `scripts/install.sh` block, `tests/hooks/aa-ma-chart-guard.bats` + 6 fixtures, `install_dry_run.bats` case, `claude-code/commands/aa-ma-plan.md` (Step 1.0, Phase 5 import, 5.3, 5.8), spec/rules/quick-ref/foundations/templates README/README/SECURITY/ATTRIBUTION/ADR-0013/CHANGELOG/TODOS, `.claude/dev/charting/writing-for-agents-eval/` + `docs/research/writing-for-agents-eval-overlap.md` (`[ad-hoc]` 6bcef05), impl-review M5 section.
- Tests: bats aa-ma-chart-guard 28/28; tests/hooks 0 failures; pytest 1099 passed / 2 skipped; shellcheck (info) all .sh rc 0; CI run 35643530138 5/5 success.
- Decisions:
  - **AD-015** — Release cut *after* §6.8, not before (M4 tagged v0.13.0 with review fixes landing post-tag). The tag now carries the fix set. Sub-step 5.5 held `IN_PROGRESS` through §6.7/§6.8 so the gate's PENDING count was 0 without claiming completion.
  - **AD-016** — `AA_MA_HOOKS_DISABLE=1` disables *enforcement* only: `fog | claim | reclaim | from-map` no-op with exit 0; `import` (a mutation the caller asked for) always runs, skipping just the clear check, and `/aa-ma-plan` Step 1.0 prints a notice when the check was skipped. Rationale (security W2): a kill switch that silently skipped the move left `/aa-ma-plan` reporting a map it never imported.
  - **AD-017** — Effort and task names are validated as `^[a-z0-9-]+$` at every entry (`slug_ok`, `map_effort` → exit 2; command fences exit 2) and the fog-test removal is `rm -f -- "$MAP"; rmdir` gated on the guard's rc 1 only. Rationale (security CRITICAL): the draft fence ran `rm -rf "$(dirname "$MAP")"` on any non-zero rc with an unvalidated `<effort>`. Both fences are executed by bats, not grepped.
  - **AD-018** — Map content is data: `$SEED` loads inside `<MAP_SEED>` with a data-not-instructions preamble; only `Decided with <user> <date>` lines are settled HITL decisions; a map's `## Notes` may widen the write scope only to `docs/**` and only after an `AskUserQuestion` confirmation (security W4, A03).
  - **AD-019** — No `vX.Y.Z` pins in rules/spec/ADR prose for unreleased work (ADR-0011/0012 style: milestone + date); `CHANGELOG.md ## Unreleased` is the only pre-release site and `release.sh` owns the number (future-proofing W2).
  - Prototype-run finding: `claim` asks the session-wide one-at-a-time invariant before `Blocked-by` (28c6844) — a blocked ticket otherwise hid the reason the frontier list exists for.
