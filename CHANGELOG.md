...
- **Living architecture doc (`diagram-generation` M6, ADR-0016)** — `docs/architecture/{README,component,plugin-surface}.md`
  are 100% generated by `codemem draw --write` from the codemem graph. `codemem draw --check` regenerates and
...
- **Plugin-surface extractor (M4)** — `codemem.draw.plugin_surface.extract()` recovers commands → skills → agents →
  hooks from `claude-code/` markdown and classifies every reference ON_DISK / DECLARED_EXTERNAL / DANGLING against
  an explicit allowlist; pinned by `tests/golden/plugin-surface.json`.
- **Captions sidecar (M5)** — `docs/architecture.captions.json`: flat, path-keyed prose plus `@start`, rendered
  outside the mermaid fence; `@start` is a highlighted node.
- **codemem schema v3 (M1)** — `file_edges` holds file-level import edges; dotted callees keep their
  import-alias qualification.
- **`aa_ma.render.graph` (M2, ADR-0014)** — read-only stdlib-sqlite3 seam onto codemem's index; import-linter
  contract `aa-ma-never-imports-codemem`, and `lint-imports` now runs in CI.

### Fixed

- **`codemem build` misattributed symbols on rebuild** — the full build deleted with foreign keys off, so orphaned
  rows re-attached to reused file ids; it now clears the graph inside the build transaction.
- **Mermaid label injection** — a file name carrying `%%{init}%%` could restyle a rendered diagram; labels now
  entity-escape `% { }` and backtick.
- **Flaky hook test** — `build_active_dir.sh` read the clock per task, so equal mtime offsets could straddle a
  second and reorder the tiebreak test.

### Security

- `security.yml` declares top-level `permissions: contents: read`, and every action is pinned by commit SHA.
...
- **`aa-ma-render`** (`src/aa_ma/render/html.py`) — markdown → one self-contained HTML
  ...
  tables; mermaid fences become `<pre class="mermaid">` drawn by mermaid 11.17.2
  loaded from jsDelivr with Subresource Integrity behind a CSP meta, `prefers-color-scheme`
...
- **ci**: correct bats-action tag — strip 'v' prefix (4.0.0 not v4.0.0)
- **claude-code**: address PR #1 review — cut §7.2.6, add tests, unblock CI
