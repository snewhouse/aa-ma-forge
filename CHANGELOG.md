...
Post-impl adversarial review feature (Phase 6.8 + `/verify-impl`), per ADR-0005.
Target release: v0.8.0. Finalised entries regenerated by `cz bump` in M6.

- feat(hooks): add `security-static-check.sh` PreToolUse hook (M2) — 5 mechanical
  security pattern classes (hardcoded secrets, shell injection, path traversal,
  SQL string concat, unsafe binary deserialisation CWE-502); bypassable via
  `AA_MA_HOOKS_DISABLE=1` or `[security-bypass: <reason>]` marker; 21/21 bats GREEN.
- feat(plan-verification): add `Audit-Profile` + `TDD-Waiver` parsers (M3) —
  Angle 6 structural checks #4 (Audit-Profile presence for v0.8.0+ plans) and #5
  (TDD-Waiver canonical values); grandfathered by `Created:` date; 58 new pytest
  cases including corpus test against 9 completed plans.
- docs(adr): add ADR-0005 (M1) — Post-Impl Adversarial Review architectural
  decision; documents 8 sub-decisions (D1-D7b); updates `aa-ma-specification.md`
  with §6.8 anatomy.
