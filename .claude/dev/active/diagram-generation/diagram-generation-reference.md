...
- Mermaid: `mermaid.io_to_mermaid(edges, level)`. Subgraph `io_<lang>`, sink node `io_<lang>_<category>`, edges `n.. eN@-->|"calls"| io_..`; `TIER_STYLE` qualified `stroke-width:2px,stroke-dasharray:0` / bare `stroke-dasharray:4 4`, applied by `class eA,eB <tier>` lines (mermaid has no `:::` on edges). [valid: 2026-09-25]
...
- Parser: ast-grep `*-call` rules (ts/tsx/js/go) capture `$CALLEE`; the wrapper emits one unresolved `CallEdge` per (innermost enclosing function/method, callee); top-level calls and call-chained callees (`fetch(x).then`) are dropped. Go methods are still skipped as orphans (M1 behaviour), so calls inside them are lost. Python: `from m import n [as k]; k()` is stored as `m.n`. [valid: 2026-09-25]
