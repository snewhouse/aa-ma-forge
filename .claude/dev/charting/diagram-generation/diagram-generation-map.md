...
- Blocked-by: 9
#### Question
Graduated from fog 2026-09-22 — **this effort created the inconsistency**. Ticket 9 rewired `understand-codebase`'s Deep tier to source diagrams from codemem, while the same skill still sources structural reuse from `PROJECT_INDEX.json`. Measured: **10 files** under `claude-code/` reference `PROJECT_INDEX`, **8 of them inside `understand-codebase`** (`SKILL.md`, `REUSE-MAP.md` ×8 mentions, `DIMENSIONS.md`, `PLAYBOOK-CONTRIBUTE.md`, `PROS-CONS-RUBRIC.md`, `DEEPDIVE-TEMPLATES.md`, `ONBOARDING-TEMPLATE.md`, `AGENTS-MD-TEMPLATE.md`), plus `impact-analysis/SKILL.md` and `system-mapping/SKILL.md`. Decide: retire `PROJECT_INDEX.json` in favour of codemem, keep both with an explicit division of labour (index for symbol lookup, codemem for edges?), or keep both and say nothing. Note `PROJECT_INDEX.json` is generated by `~/.claude-code-project-index/scripts/project_index.py` — **outside this repo**, so retiring it here does not remove the generator, and `rules/project-index-awareness.md` instructs every session to look for it.
#### Answer
**Keep both as a conditional fallback with codemem stated as the default; repoint the `/index` provisioning instructions at `codemem build`.** Grill round 15 with Ste, 2026-09-22.

**Three measurements:**
1. **codemem's MCP surface is a strict superset.** project-index exposes 6 tools — `who_calls`, `blast_radius`, `dependency_chain`, `search_symbols`, `file_summary`, `dead_code`. codemem exposes **all six** plus `co_changes`, `hot_spots`, `layers`, `owners`, `symbol_history`, `find_references`, `find_dead_code`, `aa_ma_context` (14 total). Language coverage is the same set; codemem parses bash with an ast-grep rule (`parser/rules/bash.yml`) where PROJECT_INDEX uses regex.
2. **The only data unique to `PROJECT_INDEX.json` is `dir_purposes`/`tree`, and `dir_purposes` is demonstrably wrong on this repo** — `docs/spec → "Test specifications"`, `claude-code/skills/grill-with-docs → "Project documentation"`. Heuristic labelling this repo disproves, so nothing of value is lost by preferring codemem.
3. **Retiring it from the plugin would cost Ste nothing personally**: `project-index-awareness.md` lives in `~/.claude/rules/`, while the plugin ships only `aa-ma.md` and `engineering-standards.md` in `claude-code/rules/`. The global rule is outside anything this effort touches.

**Reference shapes:** most of the ~10 files are already conditional — "When `PROJECT_INDEX.json` exists…", "preferred when … exists" (`impact-analysis/SKILL.md:225`, `system-mapping/SKILL.md:74,134`, `REUSE-MAP.md:47,84`). The exception is `REUSE-MAP.md:38` (and :47-50): *"If PROJECT_INDEX.json missing … → run `/index`"* — instructing a consumer to run a command the plugin does not ship. **That is the gap-6 shape again**, the same one Ticket 9 fixed for `/codebase-deep-dive`.

**Decisions:**
1. **Conditional fallback, codemem preferred.** The conditional references are rewritten so **codemem is the stated default** and `PROJECT_INDEX.json` is named as an equivalent fallback when present. Ste's existing workflow keeps working, a consumer without the external generator is never stranded, and no capability is lost because codemem answers all six of the index's queries. **Accepted cost:** two graph backends named in the plugin's documentation, where retirement would have left one.
2. **Provisioning instructions repoint at `codemem build`** (0.44s, shipped) instead of `/index`. ~3 edits, consistent with Ticket 9's precedent: fix the instructions that tell someone to *run* something, leave conditional reuse alone.

**Note for the plan:** `ensure_built` provisions codemem transparently on first MCP query (5s SLO), so a skill reaching codemem through the MCP door needs no build instruction at all — only the CLI paths do. The plan should not add a `codemem build` step where an MCP query already covers it.

**Not adopted:** no new `claude-code/rules/` entry for codemem awareness. `ensure_built` makes a "check the index exists" rule unnecessary, and the plugin's rule set stays at two.


## Not yet specified

<!-- Emptied 2026-09-22 by a fog-triage session with Ste, after all 16 tickets resolved.
     Of the 8 bullets: 5 moved to Out of scope with a stated reason, 3 graduated to
     Tickets 17-19. The list never grew across 16 resolutions — the map was sized right. -->
...
_Fog triage, 2026-09-22 — five bullets ruled out with reasons, so no future session re-opens them cold:_

- **Bump to mermaid 12 + `layout: elk`** — deferred by Ticket 13; blocked on mermaid-cli shipping a 12 line. Its own one-milestone decision when that lands.
- **Type/schema-flow View** (Pydantic/dataclass producers/consumers) — Q6 said "both, I/O first"; it needs codemem to track **class references**, which it does not. Revisit when that capability exists.
- **Forge-specific AA-MA-artifact data-flow View** (which modules/hooks read/write plan/tasks/reference/context-log/provenance/map) — Ticket 8 fixed the living-doc file set at `{README, component, io, plugin-surface}.md`; a fifth forge-only file is outside it.
- **Diff-scoped "what changed in this PR" reviewer view** — reviewers are named in the amended Destination, but this answers a different question and needs a commit range the current design has nowhere to accept. A named follow-on effort, not a v1 gap.
- **Per-level narrative pages** (prose + diagram) instead of diagrams with captions — settled against by Tickets 8 and 12: the files are 100% generated and prose lives in the path-keyed captions sidecar.
