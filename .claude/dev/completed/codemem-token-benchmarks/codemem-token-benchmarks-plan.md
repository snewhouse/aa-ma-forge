...
- **Task 4.2** — Update `docs/codemem/kill-criteria.md` **Signal 2** (M1 architectural kill) status line: replace the 2026-04-17 "Aider token-efficiency comparison is deferred post-M4-ship" clause with the updated status including the Aider benchmark outcome. **Key nuance — AND logic + provisional state:** Signal 2's trigger is `codemem build > 1.5× /index wall-clock ON reference repos AND PageRank projection doesn't beat Aider`. Task 4.1 cleared the first conjunct **only on the small reference repo (aa-ma-forge) at 0.73×**; the kill-criteria.md status explicitly flags medium-repo and 50k-LOC measurements as **pending user-provided reference repos** — so the first conjunct is PROVISIONAL, not PINNED. This benchmark updates the Aider sub-claim; the **composite remains PROVISIONAL DID-NOT-TRIGGER** (small-repo only) regardless of this benchmark's outcome, until the pending medium+large wall-clock measurements land. Three sub-cases: (a) codemem beats Aider at equal budget — Aider sub-claim "PASS on small repo, also pending medium+large"; (b) codemem loses to Aider at equal budget — Aider sub-claim "fails on small repo, noted as risk-signal to monitor"; (c) findings ambiguous — "provisional — see benchmark §X". Composite verdict line must preserve "pending medium+large for full verdict" in all three cases. Mode: HITL.
...
- If codemem loses the Aider sub-comparison: `context-log.md` records the event as a risk-signal (not a kill — Signal 2 composite remains PROVISIONAL DID-NOT-TRIGGER per 0.73× wall-clock on the small repo, with medium+large still pending) with a linked pointer to the root cause (tokenizer proxy / algorithm choice / etc)
...
### M4 Top 3 Risks
1. **(R1)** Findings are ambiguous (codemem wins size but loses top-symbol overlap, or vice versa). Mitigation: report both signals separately; leave the Signal 2 Aider-sub-claim in "provisional — see benchmark §X for discussion" rather than forcing a binary. Composite Signal 2 remains PROVISIONAL DID-NOT-TRIGGER (cleared only on the small repo per Task 4.1's 0.73×; medium+large wall-clock measurements are still pending per the kill-criteria.md status).
