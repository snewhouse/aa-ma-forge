...
- Acceptance Criteria:
  - [x] `claude-code/skills/aa-ma-research/SKILL.md` = Derived provenance line + frontmatter `name: aa-ma-research` + upstream HEAD body verbatim + a `## In this repo` section with the three AA-MA lines. Recipe: `sed '1d;/^## In this repo/,$d' SKILL.md | sed 's/^name: aa-ma-research$/name: research/' | md5sum` = `e1dd6af372a9e1d134eff7d8362fe3f7`; `test_aa_ma_research_frontmatter.py` green; `FORKS.json` row `state: derived`, `upstream_sha` full sha, `upstream_md5.SKILL.md: e1dd6af372a9e1d134eff7d8362fe3f7`. `stat -c '%Y' ~/.claude/skills/research` unchanged before/after `install.sh`.
  - [x] `claude-code/agents/aa-ma-researcher.md` frontmatter `tools: Read, Glob, Grep, Bash, WebSearch, WebFetch, Write`; `tests/agents/test_aa_ma_researcher_agent.py` asserts set equality, asserts `"Agent" not in tools`, and that the prompt contains "exactly one file", "cite", "Not pursued" and "never run `claude`".
  - [x] **Prototype verdict in provenance:** in a fresh session after `install.sh`, one live `Skill(aa-ma-research)` run on "Does `scripts/install.sh` back up a real directory before symlinking, and where?" produces `docs/research/mattpocock-trio-adoption-install-backup.md` where `grep -Ec '^\*\*(Created|Author|Reviewed-Through-Date|Valid-Through|Sources):\*\*' <file>` = 5 and `grep -Eq '[A-Za-z0-9_./-]+\.(sh|md|py):[0-9]+' <file>`; the Result Log pastes the agent-completion notice showing its tool tally with no `Agent` calls and zero Bash `tool_use` invoking `claude` in the subagent transcript: `jq -r 'select(.message.content|type=="array") | .message.content[] | select(.type=="tool_use" and .name=="Bash") | .input.command' ~/.claude/projects/<project-dir>/<session>/subagents/<agent>.jsonl | grep -cE 'claude (-p|--print)'` = 0 (a bare `grep -c 'claude -p'` over the whole file is self-defeating — the agent's own prompt contains that phrase); provenance line `[ts] PROTOTYPE — <Milestone 4 heading> — PASS: files=1 nested_agents=0; branch=n/a`.
  - [x] `grep -q 'Skill(aa-ma-research)' claude-code/commands/aa-ma-plan.md`; `grep -c 'research_files=' claude-code/commands/aa-ma-plan.md` ≥ 2 and `docs/spec/plan-marker-grammar.md` ≥ 2; `grep -q 'docs/research/' <(sed -n '/Step 5.3/,/Step 5.5/p' claude-code/commands/aa-ma-plan.md)`; `grep -q 'aa-ma-research' claude-code/skills/aa-ma-plan-workflow/references/PHASE_3_RESEARCH.md`; the `research-consolidation` row there contains "optional"; `grep -q '_phase_3' TODOS.md` (deferred, D1).
  - [x] `~/.claude/skills/aa-ma-research` after `scripts/install.sh` is a symlink to the repo dir (no backup needed — new name).
  - [x] Counts: skills 20→21, agents 11→12 in `SECURITY.md` (lists), `README.md` skills table, foundations `### Skills (21)`/`### Agents (12)` (CLAUDE.md locally); ADR-0012 → Implemented.
  - [x] **Release:** `scripts/release.sh minor --headline "fork manifest, grilling, prototype gate, research agent" --dry-run` clean, then real run → tag `v0.13.0` pushed; GitHub Release exists (OV4).
...
- Result Log: COMPLETE 2026-09-21 — §7.3 approved by Ste. 7/7 criteria verified (§6.1; recipe md5 e1dd6af3… unchanged after §6.8 fixes). Commits 52dfe9d → 6479d21 → 3fb7bf6 → 9ece1d4 → 0c41d2b → ca0de90 → c877314 → 3750908 → 006f80e → bdfcf8f → c27250b (bump) → 0e92092 → d50772b → 3c31c3c (red) → 252e690 → a74865d (+ milestone commit, + provenance commit). Release v0.13.0 tag → c27250b, GitHub Release published. §6.3 LOW (no src/ Python); §6.6 skipped (test-only py diff); §6.7 PASS; §6.8 PASS_WITH_WARNINGS 0/8/14 — all 8 WARNINGs fixed, 2 INFO → TODOS.md. Tests: full suite 1099 passed / 2 skipped; bats fork-drift + skip-warn 14/14; CI green on 006f80e. Deviations (recorded in context-log): 4.3 `claude -p` criterion reworded (jq tool_use filter); ADR INDEX 0011 row fixed in 4.5 (M3 drift).
...
- Acceptance Criteria:
  - `readlink ~/.claude/skills/aa-ma-research` = repo dir; `~/.claude/skills/research` unchanged; fresh session; live `Skill(aa-ma-research)` run yields `docs/research/mattpocock-trio-adoption-install-backup.md` + provenance PROTOTYPE line per the milestone criteria (agent tool tally with zero `Agent` calls; zero Bash `tool_use` invoking `claude` in the subagent transcript — jq filter per the milestone criterion).
- Artefacts: `docs/research/mattpocock-trio-adoption-install-backup.md`, `mattpocock-trio-adoption-provenance.log`.
- Result Log: ✅ COMPLETE 2026-09-21 — Mode: HITL — user approved "Proceed + fix criterion". Install: `readlink ~/.claude/skills/aa-ma-research` = `/home/sjnewhouse/projects/github_private/aa-ma-forge/claude-code/skills/aa-ma-research` ✓; `~/.claude/agents/aa-ma-researcher.md` → repo file ✓; `~/.claude/skills/research` mtime 1763649209 before/after ✓. Agent registry reloaded lazily (announced 2 turns after install.sh), dispatched in-session via `Skill(aa-ma-research)`; report delivered after `/clear` (fresh-session half). Output `docs/research/mattpocock-trio-adoption-install-backup.md` (49 lines): header count = 5 ✓; `path:line` cites ✓ — all six line refs (install.sh :119-181, :209-221, :186-187/:214, :346-358, :149, :333-344/:373) re-verified by hand against HEAD. Agent-completion notice: task `acf2c08acd9784d96`, `tool_uses=4`, `subagent_tokens=49474`, `duration_ms=69410`; transcript tally Bash×3 + SubagentHandback×1, **Agent×0**. `claude` invocations: jq filter over both transcript halves (`ad141bb3…`, `784f66c2…`/`subagents/agent-acf2c08acd9784d96.jsonl`) = 0 ✓ (bare `grep -c 'claude -p'` = 2, both the 4.2 prompt text — criterion reworded, see context-log). Bonus finding relayed: `hooks/lib/aa-ma-footer.sh` exists but install.sh never links/backs it up (L-016 (b) recurrence, out of scope). PROTOTYPE line written.
...
- Result Log: ✅ COMPLETE 2026-09-21 — Commit: [milestone commit — see provenance] — 9/9 criteria verified (1 prototype PASS tickets=4 resolved=4 refused_claims=1; 2 handoff dry-run exact first line, no task dir; 3 bats 28/28; 4 install block + readlink; 5 template + canonical 23 passed; 6 --from-map greps + import leg; 7 taxonomy 5+4 ×6 sites; 8 commands 13, hooks 8 unchanged; 9 ATTRIBUTION/ADR-0013 Implemented/CHANGELOG/TODOS/CRITICAL_PATH_REVIEW/v0.14.0 cut). §6.7 PASS; §6.8 PASS_WITH_WARNINGS (2 CRITICAL fixed, 13/13 WARNING fixed); pytest 1099 passed; bats tests/hooks 0 failures; CI green 35643530138.
...
- Result Log: ✅ COMPLETE 2026-09-21 13:56 — Mode: AFK — auto-dispatched. **TDD order:** commit 1 `b3ac7ee` = `tests/hooks/aa-ma-chart-guard.bats` (19 cases: fog ×2, claim ×5, from-map ×3, reclaim ×2, import ×4, plumbing ×3) + 4 fixtures + `install_dry_run.bats` case 5 — RED (guard absent, rc 127; install case fails). Commit 2 (this) = `claude-code/hooks/lib/aa-ma-chart-guard.sh` (bash+awk, ~200 lines; sources sibling `aa-ma-parse.sh` via `readlink -f`; `AA_MA_HOOKS_DISABLE=1` → exit 0; exit 0/1/2) + `scripts/install.sh` explicit `create_symlink` block — GREEN: `bats tests/hooks/aa-ma-chart-guard.bats` 19/19; `bats tests/hooks/install_dry_run.bats` 5/5; whole `bats tests/hooks/` 194/194; `shellcheck` on guard + install.sh rc 0; `scripts/install.sh --dry-run | grep -q aa-ma-chart-guard` OK; real install run → `readlink ~/.claude/hooks/lib/aa-ma-chart-guard.sh` = repo path. `_cand` literal: `aa-ma-chart.md` = 1 (5.1); `aa-ma-plan.md` lands in the 5.4 flag leg (next commit) per the milestone execution-order note. Design: ticket fields must precede `#### Question` (any `#` heading ends the field run); `import` re-runs `from-map` first (defence in depth, one extra call); `reclaim` = `set_status OPEN + Reclaimed` then the ordinary `claim` path (invariant re-checked).
...
- Result Log: ✅ COMPLETE 2026-09-21 — Mode: HITL — user approved at gate. **chart:** destination + scope + frontier grilled with Ste (2 AskUserQuestion rounds); map written in template grammar; `fog` rc 0; `claim ticket-1` (research) → `aa-ma-researcher` (8 `gh api` calls, 0 web fetches) → `docs/research/writing-for-agents-eval-overlap.md` (162 lines, 5 header fields) → `#### Answer` + RESOLVED. `grep -c '^- Type: research'` = 1; `'^- Type: grilling'` = 3. **work ×3 (one non-research ticket each):** Ticket 2 (fork verbatim, model-invoked), Ticket 3 (retire write-a-skill, fold sections in), Ticket 4 (graduated from fog: scribe/Phase 5 do not invoke the skill — no). **Refused claim** (Ticket 3 while Ticket 2 CLAIMED), stdout verbatim:
  ```
  refused: Ticket 2: Fork verbatim, derive, or skip? is CLAIMED — at most one non-research ticket at a time
    resolve it, or re-take it with: reclaim <map> ticket-2
  frontier:
  ```
  (frontier empty: Ticket 3 was also blocked by 2.) **Handoff:** `"$GUARD" from-map <map>` → `clear: tickets=4 resolved=4 ruled_out=0`, rc 0; the shipped Step 1.0 fence executed with `--dry-run` → first line `--from-map dry-run: effort=writing-for-agents-eval tickets=4 — no task directory created` + Decisions so far (4 lines) + `## Answers` (4 blocks), 25 lines; `test ! -d .claude/dev/active/writing-for-agents-eval` ✓. Provenance: `PROTOTYPE — <M5 heading> — PASS: tickets=4 resolved=4 refused_claims=1; branch=n/a`. **Template-amendments:** none (map-template.md unchanged); guard amendment from the run: `28c6844` hunk `do_claim` — the one-at-a-time invariant is checked before `Blocked-by` (a blocked ticket otherwise hid the session-wide reason), pinned by a new bats case. Research-note QA: the researcher's Answer prose disagreed with its own table tallies → sent back to reconcile (map Answer cites only table-stable facts). Charting artefacts committed `[ad-hoc]`: `6bcef05`.
...
- Result Log: **Split — flag leg (before 5.3) / docs leg (after 5.3).** ✅ Flag leg 2026-09-21 14:02 — Mode: AFK. `aa-ma-plan.md`: Key Features bullet; new **Step 1.0** (`_cand` guard resolution, `from-map` refusal passthrough, `TICKETS` from the guard's `clear:` line, `SEED` = Decisions so far + `## Answers`, `--dry-run` prints `--from-map dry-run: effort=<e> tickets=<N> — no task directory created` + seed and exits before 1.3); Step 5.1 `--from-map` import block (`"$GUARD" import …`); Step 5.3 reference line per RESOLVED ticket with `[valid: <date>]`; Step 5.8 summary row. Checks: `_cand` literal count = 1; `grep -q -- '--from-map <effort> \[--dry-run\]'` OK; `aa-ma-render` rc 0; **the Step 1.0 fence executed** (awk-extracted, `EFFORT=demo-effort`) in a temp git repo: clear fixture → first line exact + seed, rc 0, no `.claude/dev/active`; open fixture → guard's refusal listing, rc 1. ✅ Docs leg 2026-09-21 16:45 (after 5.3). Taxonomy 5+4: spec §II map row + `MAP -. optional .-> PLAN` edge (node declared on its own line so the literal edge form is present); `rules/aa-ma.md`, quick-ref Optional Files + command block, foundations (taxonomy row + `### Commands (13)` + row), `docs/templates/README.md` (row + prose), `README.md` (templates line 9 types, `/aa-ma-chart` row), `CLAUDE.md` local (13 commands, 9 types); `grep -rl '+ 3 optional' docs claude-code README.md` empty. Counts: `SECURITY.md` 13 command files (+aa-ma-chart); "8 hooks" unchanged. `docs/ATTRIBUTION.md`: charting paragraph (concept from wayfinder @ c55ee46, no files forked, invariant reworded OV5) + where-it-lives paths. ADR-0013 → Implemented (M5, v0.14.0) with an as-built note; component view updated to as-built nodes (guard, aa-ma-research, template). `TODOS.md`: `/aa-ma-share` map allowlist entry + `_phase_1_3`/grilling fingerprint entry. `CHANGELOG.md` M5 entry (seeded in 5.1). `map-template.md` NOT added to `WRITER_TEMPLATES`. Checks: `uv run pytest tests/commands tests/test_active_plans_canonical.py` 54 passed (count test green); `aa-ma-lint-views` plan rc 0, ADR-0013 rc 0 (render UNKNOWN — no mmdc); `--from-map` grep hits both files.
...
- Result Log: ✅ COMPLETE 2026-09-21 — Mode: HITL — Ste approved "Approve + release". Order (AD-015): CRITICAL_PATH_REVIEW (hook-modification: guard bats 20/20→28/28, shellcheck rc 0, install symlink) + §6.3 impact analysis LOW → §6.7 fence PASS → §6.8 (5 agents; 2 CRITICAL accepted+fixed e42d9e7/2bd406d, 13/13 WARNING fixed, security re-check 0/0/2, PASS_WITH_WARNINGS) → GATE APPROVAL (context-log, Decision: APPROVED) → release. Charting artefacts committed `[ad-hoc]` 6bcef05 and pushed. Circuit breaker: CI ShellCheck red since 50c8099 (SC2015 info-level on CI's shellcheck; local 0.11.0 silent) → 3f83d36 → run 35643530138 all 5 jobs success. Pre-flight: tree clean, HEAD==origin/main, `gh auth status` ok; dry-run `0.13.0 → 0.14.0` MINOR. `scripts/release.sh minor --headline "charting: pre-plan decision maps (/aa-ma-chart, --from-map)"` → bump commit b527241 (`[ad-hoc]`; CHANGELOG, README, VERSION, pyproject, uv.lock), annotated tag `v0.14.0` (remote object ea6d7d1), GitHub Release https://github.com/snewhouse/aa-ma-forge/releases/tag/v0.14.0 (draft=false). Tagged tree: pyproject + uv.lock `aa-ma` 0.14.0 (L-015 ✓), CHANGELOG `## v0.14.0 (2026-09-21)`, README `**Current version:** v0.14.0 — charting: …`.
