...
### File taxonomy (5 standard + 4 optional)
...
| `prototype` | Throwaway-prototype dispatcher: routes between LOGIC (a single self-contained HTML demo — state panel, free-play buttons, tabbed walkthroughs — for state/business-logic questions) and UI (structurally different variants on an existing route, switchable via `?variant=`) branches, and captures the result on a `prototype/<name>` branch (re-forked from mattpocock/skills @ c55ee46; operationalises engineering-standards Theme 1 `Prototype-Required: YES` — see ADR-0003, ADR-0011) |
