...
Current `/index` (`~/.claude-code-project-index/`) has solid bones — SQLite cache, monolithic JSON projection, 6 MCP tools, **and a file-level PageRank in `scripts/pagerank.py`** (verification audit V3 surfaced this; not a green-field PageRank invention on our part). Three structural debts remain: ~1500 LOC of fragile JS/TS regex; a JSON-canonical model that couples index size to context budget; no git-derived intelligence. The 2026 landscape (GitNexus 19k stars, Axon, Codegraph, jCodeMunch, CodeMCP, RemembrallMCP, DevSwarm, Code Pathfinder, Kiro CLI) shows where the category is converging — call-graph + blast-radius via tree-sitter + MCP — and where it isn't going (git mining + AA-MA-native context are uncontested).

**Sharpened differentiation vs `/index`'s existing PageRank:** ours is **symbol-level edge-weighted PageRank** over the call graph (not file-level), with **SCIP-shaped symbol IDs** for cross-tool interop and Aider-style **token-budget binary search** over the ranked output. `/index`'s PageRank ranks files; ours ranks individual symbols — strictly more granular for context injection.
...
- **Step 1.1: Project scaffold + dual-distribution packaging** (AFK) — Implement the packaging structure decided in Step 1.0. Create `claude-code/codemem/` (commands, skills, hooks, mcp/) + the codemem source tree at the location chosen in 1.0 (either `src/aa_ma/codemem/` re-exported, or `packages/codemem-mcp/src/codemem/` if subdir route). Configure pyproject so codemem is independently installable: `pip install codemem-mcp` builds a wheel with NO `aa_ma` dependency; entry points `codemem-cli`, `codemem-mcp-server`. Add `ast-grep-cli>=0.42,<0.43`, `fastmcp` to deps. **Pin verified 2026-04-13:** ast-grep-cli 0.42.1 is on PyPI with wheels for Linux+macOS+Windows manylinux/musllinux. **Append `.codemem/` to repo `.gitignore`** (per V7). Acceptance: `pip install -e .[codemem]` works; `pip install codemem-mcp` from a clean checkout produces a working wheel; `cat .gitignore | grep "\.codemem/"` returns a match; CI matrix builds wheel on Linux + macOS.
