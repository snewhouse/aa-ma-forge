...
# Extract the SHIPPED §6.7 derivation preamble verbatim and run it. A grep for
# the function name is not a guard: the previous form was an exact-literal
# negative match that would go green if a future edit merely added quotes,
# reinstating the false-PASS defect with the test still passing. Executing the
# text is the only assertion that survives a rewrite.
_run_gate_derivation() {  # $1 = dir holding <name>-tasks.md, $2 = name
    awk '/^# Resolution order matches the shipped hooks/,/^if \[\[ -z "\$\{MILESTONE_TITLE\}" \]\]; then/' \
        "$MILESTONE_CMD" > "$BATS_TMPDIR/preamble.sh"
    printf '  exit 1\nfi\nprintf "DERIVED:%%s\\n" "${MILESTONE_TITLE}"\n' \
        >> "$BATS_TMPDIR/preamble.sh"
    TASK_DIR="$1" TASK_NAME="$2" bash "$BATS_TMPDIR/preamble.sh"
}

@test "the shipped gate text derives exactly one ACTIVE milestone" {
    local d="$BATS_TMPDIR/g1"; mkdir -p "$d"; cp "$FIX_ONE" "$d/g1-tasks.md"
    run _run_gate_derivation "$d" g1
    [ "$status" -eq 0 ]
    [[ "$output" == *"DERIVED:Milestone 2: The one being gated"* ]]
}

@test "the shipped gate text refuses when two milestones are ACTIVE" {
    local d="$BATS_TMPDIR/g2"; mkdir -p "$d"; cp "$FIX_TWO" "$d/g2-tasks.md"
    run _run_gate_derivation "$d" g2
    [ "$status" -ne 0 ]
    [[ "$output" != *"DERIVED:"* ]]
    # The refusal must name both, or it is unactionable.
    [[ "$output" == *"Milestone 2: Older milestone left ACTIVE"* ]]
    [[ "$output" == *"Milestone 4: The one actually being gated"* ]]
}

@test "the shipped gate text refuses when no milestone is ACTIVE" {
    local d="$BATS_TMPDIR/g3"; mkdir -p "$d"; cp "$FIX_NONE" "$d/g3-tasks.md"
    run _run_gate_derivation "$d" g3
    [ "$status" -ne 0 ]
    [[ "$output" != *"DERIVED:"* ]]
}

@test "gate condition 1 halts on a dirty task dir instead of printing PASS" {
    # `# HALT` was a comment: the gate printed BLOCKED and then
    # "ENG-STANDARDS-GATE: PASS (all 5 conditions satisfied)" with exit 0.
    local d="${REPO_ROOT}/.claude/dev/active/batsdirty$$"
    mkdir -p "$d"; cp "$FIX_ONE" "$d/batsdirty$$-tasks.md"
    awk '/^### 6\.7/{s=1} /^### 6\.8/{s=0} s' "$MILESTONE_CMD" \
        | awk '/^```bash$/{f=1;next} /^```$/{f=0;print "";next} f' > "$BATS_TMPDIR/full67.sh"
    run env TASK_NAME="batsdirty$$" bash "$BATS_TMPDIR/full67.sh"
    rm -rf "$d"
    [ "$status" -ne 0 ]
    [[ "$output" == *"BLOCKED"* ]]
    [[ "$output" != *"PASS (all 5 conditions satisfied)"* ]]
}

@test "the three functions changed in this window agree under mawk and gawk" {
    load_helper
    local tested=0 bin shim
    for bin in gawk mawk; do
        command -v "$bin" >/dev/null 2>&1 || continue
        # Unique per-run path. BATS_TMPDIR is /tmp on this host and persists
        # between runs, so a fixed name collides with the shim written by
        # "extractor produces identical results under mawk and gawk" — and a
        # symlink left there makes that test's `printf > $shim/awk` follow
        # through to the real binary and die with EACCES. Caught by the
        # mutation run for this very sub-step, three tests later.
        shim="$(mktemp -d)"
        ln -sf "$(command -v "$bin")" "$shim/awk"
        [ "$(PATH="$shim:$PATH" bash -c ". '$HELPER'; aa_ma_active_milestone_strict '$FIX_ONE'")" \
            = "Milestone 2: The one being gated" ]
        [ "$(PATH="$shim:$PATH" bash -c ". '$HELPER'; aa_ma_extract_active_milestone '$FIX_ONE'")" \
            = "Milestone 2: The one being gated" ]
        [ -z "$(PATH="$shim:$PATH" bash -c ". '$HELPER'; aa_ma_extract_active_step '$FIX_NONE'")" ]
        rm -rf "$shim"
        tested=$((tested + 1))
    done
    # Non-vacuity: a shim that silently matched nothing would pass every check.
    [ "$tested" -eq 2 ]
}
...
# ---------------------------------------------------------------------------
# One H2 predicate and one recogniser (sub-step 4.12)
#
# Four spellings of "is this an H2" existed: /^## / (x2), /^##[[:blank:]]/ (x2),
# /^##[^#]/. On a tab-separated heading the library contradicted itself — strict
# read it correctly while aa_ma_extract_active_step returned the PREVIOUS
# milestone's last sub-step, the exact defect 4.8 claims to close.
#
# Separately aa_ma_is_milestone_heading, the designated SSoT predicate that
# test_grammar_parity.py pins Python against, had ZERO callers in shipped code:
# 4.7's declared Action was "rewire it to aa_ma_is_milestone_heading" and the
# implementation inlined a fourth copy of its body instead.
# ---------------------------------------------------------------------------

@test "a tab-separated H2 closes a block in every reader" {
    load_helper
    local f="$BATS_TMPDIR/tabh2-$$.md"
    printf '## Milestone 1: A\n\n- Status: COMPLETE\n\n### Sub-step 1.1: done\n\n- Status: COMPLETE\n\n##\tMilestone 2: B\n\n- Status: ACTIVE\n\n### Sub-step 2.1: real\n\n- Status: PENDING\n' > "$f"
    [ "$(aa_ma_extract_active_step "$f")" = "Sub-step 2.1: real" ]
    [ "$(aa_ma_extract_active_milestone "$f")" = "Milestone 2: B" ]
    [ "$(aa_ma_active_milestone_strict "$f")" = "Milestone 2: B" ]
}

@test "a bare '##' line closes a block" {
    load_helper
    local f="$BATS_TMPDIR/bareh2-$$.md"
    printf '## Milestone 1: T\n\n- Status: COMPLETE\n\n##\n\n- Status: ACTIVE\n' > "$f"
    run aa_ma_active_milestone_strict "$f"
    [ "$status" -eq 1 ]
}

@test "aa_ma_is_milestone_heading is actually used by shipped code" {
    # A predicate nothing calls is a guard that can never fire (L-1214). The
    # library's own comment says it exists "rather than callers re-deriving it".
    grep -q "aa_ma_ms_title\|aa_ma_is_milestone_heading" "$HELPER"
    # Every milestone-recognition site must go through the shared awk body.
    [ "$(grep -c 'sub(mre, "", t)' "$HELPER")" -le 1 ]
}
