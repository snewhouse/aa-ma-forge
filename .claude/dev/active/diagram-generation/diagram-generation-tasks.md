...
- Status: COMPLETE
- Mode: AFK
- Result Log: Mode: AFK — auto-dispatched. `mcp_tools.diagram()` (reuses `_exceeds_budget`/`_DEFAULT_BUDGET`, `cut`/`from_edges`/`to_mermaid`; binary-search truncation) + server.py registration (13 canonical). 46/46 MCP tests; full pytest 1596 passed / 2 skipped; ruff clean; lint-imports 4 kept. Live over the MCP protocol (fastmcp Client, in-memory) on the MRS scratch index: 15 tools listed; `{}` → L2 69n/97e; `L3` → L2 collapsed_from L3; scoped L3 on one skill-category dir → L3 188e dropped 294 json 31913 ≤ 32000; `L1` → 0/0. AC1: `scripts/measure_mrs.sh` → 4 MRS_BUDGET lines, second run `cmp` byte-identical; lines + sha in reference.md. Tool counts 12→13 in SECURITY.md, claude-code/codemem/README.md (+ diagram table), packages/codemem-mcp/{README.md,pyproject.toml}, docs/codemem/install-zero-config.md. §13: `MCP -->|imports inside a function| CUT/MM`, `MMRS` node; lint sigils edges=13 checked=13 phantom=0. regen: stamps only.

### Sub-step 13.4: [impl] Deep tier rewire; `.gitignore` append; PROJECT_INDEX repoint
- Status: COMPLETE
- Mode: AFK
- Result Log: Mode: AFK — auto-dispatched; edit set decided by Ste (context-log 2026-09-26). RED f9be158 (tests/skills/test_understand_codebase_rewire.py, 25: 7 named /codebase-deep-dive runs + 11 named /index runs gone, conditional reuse kept, no `.mmd` in 12 rewired files, every PROJECT_INDEX line names codemem (AC7), provisioning says `codemem build`, rules stay at 2 (AC6), write footprint declared, living-doc fence run twice in a scratch git repo → 1 `.gitignore` line + docs/architecture/ + no `.codemem` in git status (AC4)). GREEN: understand-codebase SKILL.md (Deep tier writes the living doc — new `#### Living architecture doc` fence: AA_MA_ROOT from the installed symlink, idempotent `.gitignore` append that never joins a last line, `codemem build` + `codemem draw --write`; Standard links docs/architecture/ or embeds MCP `diagram`; footprint bullet; `/index` → `codemem build`; also dropped the 'only run /codebase-deep-dive when absent' clause), references REUSE-MAP/DIMENSIONS/DEEPDIVE-TEMPLATES/ONBOARDING-TEMPLATE/AGENTS-MD-TEMPLATE/PROS-CONS-RUBRIC/PLAYBOOK-CONTRIBUTE, templates/onboarding-team.md (T6 living-doc replaces deep-dive), agents/codebase-onboarding-synthesizer.md, impact-analysis + system-mapping (codemem default; blast_radius direction difference stated — codemem downstream, project-index upstream). codebase-onboarding-health.md left: its 2 deep-dive refs are conditional/descriptive. Plugin surface: DANGLING 4 → 2 (skill:codebase-deep-dive, skill:index gone), pin updated in test_plugin_surface.py; golden regenerated (−3 edges incl. understand-codebase→general-purpose with the removed row). skills tests 140/140; xrefs + frontmatter 6/6 (no references/ file added/removed); full pytest green; lint --coverage clean; Contract amended (+ onboarding-team.md, + measure_mrs.sh).
