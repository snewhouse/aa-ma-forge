...
_Last Updated: 2026-09-26 (M1–M13 facts sections below)_
...
## M13 facts (2026-09-26)

MRS_BUDGET repo=medical-research-skills sha=efafac209f3690c02f1ffc0f297f91c89747df69
MRS_BUDGET L0 nodes=0 edges=0 chars=13
MRS_BUDGET L1 nodes=0 edges=0 chars=13
MRS_BUDGET L2 nodes=69 edges=97 chars=9099
MRS_BUDGET L3 nodes=522 edges=500 chars=77520

- The four level lines are regenerated byte-identically by `scripts/measure_mrs.sh ~/projects/github_private/medical-research-skills efafac2` (verified twice, `cmp`). Whole-repo `codemem draw --level Ln` from `git archive <sha>`, tests excluded; `chars` = mermaid characters; L3 is capped at `MAX_EDGES` (2793 more dropped). Budget 8000 tokens = 32000 json chars. [valid: 2026-09-26]
- API: `codemem.mcp_tools.diagram(db_path, *, level="L2", scope=None, hops=1, budget=_DEFAULT_BUDGET) -> {"mermaid", "level", "nodes", "edges", "dropped", "collapsed_from", "error"}` — every key always present; errors (bad level, hops<0, schema < v3 naming `codemem build`, sqlite error) are dicts with zero counts. [valid: 2026-09-26]
- Policy (13.1, Ste): over budget → collapse to the next coarser level only while it still has edges; else truncate the current level to the largest sorted `(src, dst, kind)` prefix that fits; `dropped` = maxEdges cap + budget truncation. Budget = `_exceeds_budget` over the whole json payload. [valid: 2026-09-26]
- Registration: `claude-code/codemem/mcp/server.py` `CANONICAL_TOOL_NAMES` has 13 names (+ `diagram`); server lists 15 tools with the 2 aliases. `draw.cut` / `draw.mermaid` are imported inside `diagram()` — §13 labels those edges in prose. [valid: 2026-09-26]
